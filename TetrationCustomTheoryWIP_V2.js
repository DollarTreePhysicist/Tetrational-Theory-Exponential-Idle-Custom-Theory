...
var getSecondaryEquation = () => theory.latexSymbol + "= 10\\uparrow 10^{-4} slog_{10}(\\rho)";
...
var getTetration = (base, tet) => {
	let result = BigNumber.from(0);
	if (tet > 2) {
		let a = Math.log10(base);
		let newDepth = BigNumber.from(tet + getSLog10(BigNumber.from((base))) - 1);
		return BigNumber.fromComponents(1, Math.floor(newDepth), 								getSLog10(BigNumber.from(base)));	
	}
	let remainder = tet - Math.floor(tet);
	if (remainder > 0) {
		let towerCap = remainder;
		result = base.Pow(towerCap);
	} else if (remainder <= 0) {
...
var getTau = () => BigNumber.from(10).Pow(0.0001 * getSLog10(currencyRho.value) );
